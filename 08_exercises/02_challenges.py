...
def is_valid_mobile_number(mobile_number):
    if not mobile_number[1:].isdigit():
        ...
        if mobile_number.startswith('07') and len(mobile_number) == 11:
            return True
        elif mobile_number.startswith('+447') and len(mobile_number) == 13:
            return True
        elif mobile_number.startswith('00447') and len(mobile_number) == 14:
            return True
        else:
            return False
...
def sum_digits_from_string(string):
    sum_of_digits = 0
    list_of_string = [character for character in string]
...
def get_williams(names):
    list_of_williams = []
    for name in names:
        list_split_name = name.split()
        last_name = list_split_name[1]
        if last_name == 'Williams':
            list_of_williams.append(name)
    return list_of_williams
    pass
...
def largest_number(number):
    ...
    list_of_number = [character for character in string_of_number]
    reversed_sorted_list_of_number = sorted(list_of_number, reverse = True)
    reversed_sorted_string_of_number = ''.join(reversed_sorted_list_of_number)
    return int(reversed_sorted_string_of_number)
    pass
    ...
    if len(orchard) <= 2:
        ...
        for fruit in orchard:
            if orchard.count(fruit) == 1:
                wrong_way_fruit = fruit
        return orchard.index(wrong_way_fruit)
    pass
